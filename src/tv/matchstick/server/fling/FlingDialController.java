...
public class FlingDialController implements FlingSocketListener, IController {
    ...
    @Override
    public void stopApplicationInternal(String sessionId) {
            ...
            public void run() {
                try {
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (response == 200) {
                                mFlingSrvController.onRequestStatus(0); // TODO:
                                                                        // 0 is
                                                                        // success
                                stopHeartbeat();
                            } else {
                                mFlingSrvController.onRequestStatus(1); // TODO:
                                                                        // 1 is
                                                                        // failed
                            }
                        }
    ...
    @Override
    public void joinApplicationInternal(String applicationId, String sessionId) {
        launchApplication("join", applicationId);
    }
    ...
    @Override
    public void launchApplicationInternal(String applicationId, String param,
            final boolean relaunch) {
        if (mApplicationState != null
                && !"stopped".equals(mApplicationState.state)) {
            if (relaunch) {
                launchApplication("relaunch", applicationId);
            } else {
                joinApplication(applicationId, "");
            }
        } else {
            launchApplication("launch", applicationId);
        }
        // launchApplication(relaunch ? "relaunch" : "launch", applicationId);
    }
    ...
    public void generateId() {
        android.util.Log.d("uuuuuuuuuuuuuuuu", "generateId");
        synchronized (controlerId) {
            controlerId = Integer.valueOf(1 + controlerId.intValue());
        }
        android.util.Log.d("uuuuuuuuuuuuuuuu", "generateId: " + controlerId);
    }

    @Override
    public void releaseReference() {
        android.util.Log.d("uuuuuuuuuuuuuuuu", "releaseReference");
        synchronized (controlerId) {
            boolean flag;
        ...
        android.util.Log.d("uuuuuuuuuuuuuuuu", "releaseReference: " + controlerId);
    }
    ...
    @Override
    public void reconnectToDevice(String lastAppId, String sessionId) {
        // TODO Auto-generated method stub

    }

    @Override
    public void launchApplication(String applicationId, String sessionId,
    ...
    public void onMessageReceived(String message) {
        android.util.Log.d("QQQQQQQQQQQQQ", "dial message = " + message);
