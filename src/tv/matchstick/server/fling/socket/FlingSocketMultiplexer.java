		...
		while (!multiplexer.mQuitLoop) {
			...
			if (multiplexer.mReadyToConnect.getAndSet(false)) {
				synchronized (multiplexer.mAllFlingSocketList) {
					Iterator iterator3 = multiplexer.mAllFlingSocketList
							.iterator();
					while (iterator3.hasNext()) {
						FlingSocket flingSocket = (FlingSocket) iterator3
								.next();
						try {
							flingSocket.startConnecd()
			...
			if (!errorFlingSocketList.isEmpty()) {
				for (Iterator iterator2 = errorFlingSocketList.iterator(); iterator2
						.hasNext(); ((FlingSocket) iterator2.next())
						.onConnectError())
					;
			...
			synchronized (multiplexer.mRegistedFlingSocketList) {
				Iterator iterator = multiplexer.mRegistedFlingSocketList
						.iterator();
				...
				while (iterator.hasNext()) {
					...
					if (socketchannel == null
							...
							|| !flingSocket
									.checkInterestOps(socketchannel
											.keyFor(multiplexer.mSelector),
											elapsedRealtime_l1)) {
						iterator.remove();
					} else {
						boolean flag2;
			...
			boolean flag1;
			if (multiplexer.mRegistedFlingSocketList.isEmpty()
					&& multiplexer.mAllFlingSocketList.isEmpty())
				flag1 = true;
			else
				flag1 = false;
			...
			if (i1 != 0) {
				...
				Iterator iterator1 = multiplexer.mSelector.selectedKeys()
						.iterator();
