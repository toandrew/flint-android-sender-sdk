...
import tv.matchstick.server.flint.ssdp.SSDP.ParsedDatagram;
        ...
        try {
            ...
            if (deviceId != null) {
                synchronized (mScannerData) {
                    ...
                    flintDevice = FlintDevice.Builder.create((String) deviceId,
                            address);
                    FlintDevice.setFriendlyName(flintDevice,
                            device.friendlyName);
                    FlintDevice.setModelName(flintDevice, device.modelName);
                    FlintDevice.setDeviceVersion(flintDevice, "02");
                    FlintDevice.setServicePort(flintDevice,
                            Integer.valueOf(port));
                    FlintDevice.setIconList(flintDevice, iconList);
